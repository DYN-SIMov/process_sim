...
class BinaryInteractionParametersRegression(): 
    ...
    def __init__(self,
        ...
        if activity_model.upper() == 'WILSON':
            self.activity_model_backend = WilsonActivityModel()
        elif activity_model.upper() == 'NRTL':
    ...
    def regress_BIP_parameters(self) -> None:
        ...
        for data_set in self.data_set:
            for k in range(len(data_set['x_data'])):
                ...
                args = (data_set['y_data'][k],
                        data_set['x_data'][k],
                        saturation_pressure_Pa_1,
                        saturation_pressure_Pa_2,
                        data_set['pressure_Pa'][k])

                result = minimize(fun = self.activity_model_backend.objective_function,
                                  x0 = self.activity_model_backend.initial_guess(initial_guess=opt_results[-1] 
                                                                                 if len(opt_results) > 0 else None),
                                  args = args,
                                  method = 'SLSQP',
                                  bounds=((1e-3, None), (1e-3, None)))

                opt_results.append(result.x)
